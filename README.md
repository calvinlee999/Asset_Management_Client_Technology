...
**For NAIM Support Agent Co-Pilot** (Kathleen, Stuart & Shaw — all three panel members), see [SUPPORT_AGENT_COPILOT_NAIM.md](SUPPORT_AGENT_COPILOT_NAIM.md) for the Nomura Agent Intelligence Mesh: Salesforce Service Cloud LWC + Amazon Bedrock Agents + SageMaker JumpStart internal Co-Pilot that reduces MTTR by 60%, increases FCR to 59%, and eliminates Tier-1 institutional escalations. Features three use cases (Context-Aware Troubleshooting, Historical Ticket Synthesis, Sentiment-Driven Triage), Bedrock Guardrails PII/groundedness controls, SageMaker Pipelines DAG for frustration scoring, and a cross-platform data exchange loop with NCIE and the AI Chatbot. Shares the same OpenSearch Serverless cluster as the external AI agent — one infrastructure investment, two surfaces. 3-cycle evaluation: [Cycle 1](evaluation/EVALUATION_NAIM_CYCLE_1.md) | [Cycle 2](evaluation/EVALUATION_NAIM_CYCLE_2.md) | [Cycle 3 — Certified 9.87/10 ✅](evaluation/EVALUATION_NAIM_CYCLE_3_FINAL_CERTIFICATION.md).

**For NAPCE Agentic Workflow Automation** (Kathleen, Stuart & Shaw — all three panel members), see [AGENTIC_WORKFLOW_AUTOMATION_NAPCE.md](AGENTIC_WORKFLOW_AUTOMATION_NAPCE.md) for the Nomura Agentic Proposal & Compliance Engine — Amazon MWAA orchestration + Saga distributed transactions + Bedrock Agents (DDQ, RFP, Compliance) + SageMaker GIPS Engine + Textract Document Intelligence + Amazon Batch Monte Carlo simulations. Delivers 90% reduction in RFP/DDQ response times, zero-defect continuous GIPS compliance monitoring, and fully automated 6-step proposal pipeline with 100% Saga rollback coverage. 3-cycle evaluation: [Cycle 1](evaluation/EVALUATION_NAPCE_CYCLE_1.md) | [Cycle 2](evaluation/EVALUATION_NAPCE_CYCLE_2.md) | [Cycle 3 — Certified 9.90/10 ✅](evaluation/EVALUATION_NAPCE_CYCLE_3_FINAL_CERTIFICATION.md).

**For Customer-Centric Data Strategy: CQRS Data Mesh** (Kathleen & Stuart), see [CUSTOMER_CENTRIC_DATA_STRATEGY_CQRS.md](CUSTOMER_CENTRIC_DATA_STRATEGY_CQRS.md) for the Nomura Client Technology unified data architecture — Amazon MSK Kafka + Snowpipe Streaming + MWAA + Snowflake Cortex AI + Zero-Copy Salesforce integration. Implements Command Query Responsibility Segregation (CQRS) across the full medallion stack (Bronze → Silver → Gold), delivering 360-degree client visibility with <500ms p95 latency, 30-day AUM churn early warning (vs. T+3 legacy), sub-2-second real-time data freshness across all client touchpoints, and 84% data infrastructure cost reduction ($4.4M → $700K/year). BIAN domain-aligned across Customer & Party Management, Sales & Distribution, Investment Portfolio Management, Client Reporting, and Data & Integration. The foundational data layer for NCIE, AI Chatbot, NAIM, and NAPCE — one write path, one read path, universal governance. 3-cycle evaluation: [Cycle 1](evaluation/EVALUATION_CQRS_DATA_MESH_CYCLE_1.md) | [Cycle 2](evaluation/EVALUATION_CQRS_DATA_MESH_CYCLE_2.md) | [Cycle 3 — Certified 9.85/10 ✅](evaluation/EVALUATION_CQRS_DATA_MESH_CYCLE_3_FINAL_CERTIFICATION.md).

**For Customer-Centric AI Strategy: Five Business Cases on CQRS Data Mesh** (Kathleen & Stuart), see [CUSTOMER_CENTRIC_AI_STRATEGY.md](CUSTOMER_CENTRIC_AI_STRATEGY.md) for the unified AI strategy spanning five domain-aligned business cases on the CQRS Data Mesh: A1 (Sales Cross-Selling — Agentforce + Bedrock + SageMaker JumpStart + Seismic DAM), A2 (NAIM Support Co-Pilot — LWC + Bedrock RAG + SageMaker Llama 3.1 8B + XGBoost), A3 (Personalized Portfolio — Snowflake Cortex COMPLETE `llama3-70b`, zero egress), B (NAPCE Agentic Compliance — MWAA Saga + Bedrock Agents + SageMaker GIPS Engine), and C (NCIE Product Insight — Bedrock Claude + Comprehend + OpenSearch + EventBridge). Total AI infrastructure: ~$80K/year. KPIs: 30% cross-sell conversion increase, 50% NPS/CSAT/CES improvement (+17 NPS disaggregated by case), 90% RFP/DDQ turnaround reduction. BIAN-aligned across 5 domains; HITL-gated; Zero-Trust; S3 COMPLIANCE-mode 7-year GIPS/SEC audit. 3-cycle evaluation: [Cycle 1 — 8.49/10](evaluation/EVALUATION_AI_STRATEGY_CYCLE_1.md) | [Cycle 2 — 9.31/10](evaluation/EVALUATION_AI_STRATEGY_CYCLE_2.md) | [Cycle 3 — Certified 9.86/10 ✅](evaluation/EVALUATION_AI_STRATEGY_CYCLE_3_FINAL_CERTIFICATION.md).

**For Asset Management Reports & Use Cases: Data-as-a-Product** (Kathleen & Stuart), see [ASSET_MANAGEMENT_REPORTS.md](ASSET_MANAGEMENT_REPORTS.md) for the comprehensive Data-as-a-Product framework operationalising the "Data-as-a-Product" mandate across 10 business use cases (Institutional Due Diligence & Mandate Conversion, CRM-Driven Distribution Enablement, Client Onboarding Automation, Client Reporting Modernization, Portfolio Transparency & Attribution, Wealth/Affluent Self-Service Portal, Advisor/Wholesaler Engagement Enablement, Regulatory Reporting & Compliance, Sales & Product Analytics, Data Centralization & Single Source of Truth), 10 core reports and dashboards (Pipeline & Conversion, CRM Engagement Scorecard, Performance Reporting Hub, Portal Analytics, Sales Enablement Insights, Onboarding Velocity, Compliance & GIPS Tracking, Product Strategy, Portfolio Risk & Exposure, Data Quality Scorecards), and 4 strategic domains (Institutional Sales & Wealth Advisory — Revenue Engine; Client Servicing & Operations — Experience Engine; Compliance/Risk/Product Strategy — Trust Engine; Quantitative & Hedge Fund Partnerships — Alpha Engine). KPIs: 30% client churn reduction, 90% institutional reporting automation (RFP/GIPS), 50% NPS/CES improvement. Technology: Snowflake CQRS Gold tables + Vermilion/Power BI + Salesforce FSC + Agentforce + Seismic + NAPCE GIPS Engine + Amazon Bedrock + Cognito RLS + S3 COMPLIANCE-mode 7-year WORM. Proactive Lemons Table (6 mitigations). 3-cycle evaluation: [Cycle 1 — 8.57/10](evaluation/EVALUATION_ASSET_MGMT_REPORTS_CYCLE_1.md) | [Cycle 2 — 9.36/10](evaluation/EVALUATION_ASSET_MGMT_REPORTS_CYCLE_2.md) | [Cycle 3 — Certified 9.84/10 ✅](evaluation/EVALUATION_ASSET_MGMT_REPORTS_CYCLE_3_FINAL_CERTIFICATION.md).
